...
function AddOption(id, title, description, button_text)
	if ui ~= 0 then
		Delay(200, InsertOption, id, title, description, button_text, uiUID)
	end
end
AddRemoteEvent("KUI:AddOption", AddOption)

function InsertOption(id, title, description, button_text, uid)
	if loaded then
		if uid == uiUID then
			ExecuteWebJS(ui, "InsertOption('"..tostring(id).."','"..title.."','"..description.."','"..button_text.."');")
		end
	else
		Delay(200, InsertOption, id, title, description, button_text, uid)
	end
end
